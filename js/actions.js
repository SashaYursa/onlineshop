$(document).ready(function () {
  ...
  function getPrices() {
    $.ajax({
      ...
      success: function (data) {
        ...
        document.getElementById('price-min').value = data['minPrice'];
        document.getElementById('price-max').value = data['maxPrice'];
        return data;
      }
    })
  }
  ...
  */
  ...
  $("body").delegate(".categoryhome", "click", function (event) {
    if (location.pathname != '/store.php') {
      location = 'store.php';
    }
    $("#get_product").html("<h3>Loading...</h3>");
    event.preventDefault();
