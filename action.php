                        ...
                        <div class='col-md-4 col-xs-6' >
								<a href='product.php?p=$pro_id'><div class='product'>
									...
									<div class='product-body'>
										...
										<h4 class='product-price header-cart-item-info'>$pro_price грн</h4>
...
if (isset($_POST["get_seleted_Category"]) || isset($_POST["selectBrand"]) || isset($_POST["search"]) || isset($_POST['maxPrice'])) {
    if (isset($_POST['minPrice']) and isset($_POST['maxPrice']) and isset($_POST['get_seleted_Category']) and isset($_POST['cat_id'])) {
        $id = $_POST["cat_id"];
        $minPrice = $_POST['minPrice'];
        $maxPrice = $_POST['maxPrice'];
        $sql = "SELECT * FROM products,categories WHERE product_cat = '$id' AND product_cat=cat_id AND product_price BETWEEN $minPrice AND $maxPrice";
    } else {
        if (isset($_POST['minPrice']) and isset($_POST['maxPrice'])) {
            $minPrice = $_POST['minPrice'];
            $maxPrice = $_POST['maxPrice'];
            $sql = "SELECT * FROM products,categories WHERE product_price BETWEEN $minPrice AND $maxPrice AND product_cat=cat_id";
        } else {
        ...
        echo "
                        ...
                        <div class='col-md-4 col-xs-6'>
								<a href='product.php?p=$pro_id'><div class='product'>
									...
									<div class='add-to-cart'>
										<button pid='$pro_id' id='product' href='#' tabindex='0' class='add-to-cart-btn'><i class='fa fa-shopping-cart'></i>Додати в кошик</button>
									</div>
								</div>
							</div>
			";
    ...
    if (isset($_SESSION["uid"])) {
        ...
        if (mysqli_num_rows($query) > 0) {
            echo "
					<div class='alert alert-warning'>
							<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
							<b>Товар вже додано в кошик Продовжити покупки..!</b>
					</div>";
            exit();
        }
        $sql = "INSERT INTO `cart`
...
if (isset($_POST["Common"])) {
    if (isset($_SESSION["uid"])) {
       ...
       a.product_desc,
       b.id,
       b.qty FROM products a,cart b WHERE a.product_id=b.p_id AND b.user_id='$_SESSION[uid]'";
    } else {
    ...
    a.product_desc,
    b.id,
    b.qty FROM products a,cart b WHERE a.product_id=b.p_id AND b.ip_add='$ip_add' AND b.user_id < 0";
    }
    $query = mysqli_query($con, $sql);
    if (isset($_POST["getCartItem"])) {
        ...
        if (mysqli_num_rows($query) > 0) {
                ...
                echo '
                    ...
                    <div class="product-widget">
												...
												<div class="product-body">
													...
													<h4 class="product-price"><span class="qty">' . $n . '</span>' . $product_price . 'грн</h4>
												</div>
            ...
            echo '<div class="cart-summary">
				    <small class="qty">' . $n . ' Елемент(и) вибрано</small>
				    <h5>' . $total_price . ' грн</h5>
				</div>'
            ?>
        ...
        if (mysqli_num_rows($query) > 0) {
	               ...
	               <table id="cart" class="table table-hover table-condensed" id="">
    				<thead>
						<tr>
							...
							<th style="width:7%" class="text-center">Проміжний підсумок</th>
            ...
            echo '</tbody>
				<tfoot>
					...
					<tr>
						<td><a href="store.php" class="btn btn-warning"><i class="fa fa-angle-left"></i> Продовжити покупки</a></td>
            ...
            if (!isset($_SESSION["uid"])) {
                echo '
							...
							<a href="" data-toggle="modal" data-target="#Modal_register" class="btn btn-success">Готовий до оплати</a></td>
								</tr>
							</tfoot>
                ...
                if (isset($_SESSION["uid"])) {
                    ...
                    echo
                        '<input type="hidden" name="return" value="http://localhost/myfiles/public_html/payment_success.php"/>
									...
									<input type="submit" id="submit" name="login_user_with_product" name="submit" class="btn btn-success" value="Готовий до оплати">
									</form></td>
